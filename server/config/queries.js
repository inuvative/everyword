...
//	var uniqUsers = remove_dupes([ u._id ].concat(allMembers).concat(following));		
	var oldEntries=db.feeds.find({owner:u._id}).toArray()[0].entries;	
	oldEntries = oldEntries.map(function(e){return e._id});
	var fe = db.feedentries.find({$or:[{comment : {$in : oldEntries}},{media : {$in: oldEntries}},{reference : {$in: oldEntries}}]}).toArray();
	var entries=[];
	for(var n in fe) {
		var f = fe[n];
